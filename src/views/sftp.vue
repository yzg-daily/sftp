        ...
        <qy-card class="group fixed  right-1.5 bottom-1.5" @mouseenter="mouseenter" @mouseleave="mouseleave">
          <transition @mouseenter.prevent>
            <ul @mouseenter.prevent class="server-list w-[50px] transition-all translate-y-[-8px]" v-show="show">
              <li is="qy-card" v-for="(el, index) in list" :key="el.location + index" @contextmenu="(event) => onRightClick(event, el, index)"
              ...
              <li is="qy-card" class="icon-item  relative transition w-full h-full box p-2 mb-2" @click="visible = true" v-show="!visible">
                <img @mouseenter.prevent class="transition scale-90 w-full h-full hover:scale-100" src="../assets/image/plus-sign.png" alt="">
              </li>
            </ul>
          </transition>
          ...
          <div class="rounded-2xl h-[50px] w-[50px] p-2 bg-blue-600 relative z-10" @mouseenter.prevent @contextmenu="handleRightClick">
        ...
        <transition name="add">
          <qy-card v-show="visible" v-mouse class="transition-all add-form w-[350px] fixed right-[70px]  bottom-1.5">
            <div class="tools justify-end">
              <el-icon @click="handleClose" class="rounded-2xl"><CircleCloseFilled /></el-icon>
            </div>
